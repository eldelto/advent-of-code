package net.eldelto.aof.`2020`.`04`
...
fun main() {
  ...
  // Example
  val examplePassports = exampleInput.split("\n\n").mapNotNull(Passport::parse)
  println("Example result: ${examplePassports.size}")

  // Scenario 1
  val passports = input.split("\n\n").mapNotNull(Passport::parse)
  println("Scenario 1 result: ${passports.size}")

  // Scenario 2
  println("Scenario 2 result: ${passports.filter { it.isValid() }.size}")
}

data class Height(val value: Int, val unit: String?) {
  companion object {
    private val regex = "(\\d+)(\\w*)".toRegex()

    fun parse(rawHeight: String): Height? {
      val result = regex.find(rawHeight)
      val value = parseInt(result?.groups?.get(1)?.value)
      val unit = result?.groups?.get(2)?.value
    ...
    private fun parseInt(rawInt: String?): Int? {
      return try {
...
class Passport(
  ...
  val issueYear: LocalDate,
  val expirationYear: LocalDate,
  val height: Height,
  ...
  private fun isValidYear(year: LocalDate, from: Int, to: Int): Boolean {
    return year.isAfter(LocalDate.of(from - 1, 1, 1))
      && year.isBefore(LocalDate.of(to + 1, 1, 1))
  }

  private fun isValidHeight(height: Height): Boolean {
    return when (height.unit) {
      "cm" -> height.value in 150..193
      "in" -> height.value in 59..76
      else -> false
    }
  }

  private fun isValidHex(hex: String): Boolean = hex.matches("#[0-9a-f]{6}".toRegex())

  private fun isValidColor(hex: String): Boolean = hex.matches("amb|blu|brn|gry|grn|hzl|oth".toRegex())

  private fun isValidId(id: String): Boolean = id.matches("\\d{9}".toRegex())

  fun isValid(): Boolean {
    return isValidYear(birthYear, 1920, 2003)
      && isValidYear(issueYear, 2010, 2020)
      && isValidYear(expirationYear, 2020, 2030)
      && isValidHeight(height)
      && isValidHex(hairColor)
      && isValidColor(eyeColor)
      && isValidId(id)
  }

  companion object {
    fun parse(rawData: String): Passport? {
      val rawPassport = rawData.split("[\\s|\n]".toRegex())
        .map(String::trim)
        .map { it.split(":") }
        .filter { it.size == 2 }
        .associate { Pair(it[0], it[1]) }

      return parse(rawPassport)
    }

    fun parse(rawData: Map<String, String>): Passport? {
      val birthYear = parseYear(rawData["byr"])
      val issueYear = parseYear(rawData["iyr"])
      val expirationYear = parseYear(rawData["eyr"])
      val height = rawData["hgt"]?.let { Height.parse(it) }
      val hairColor = rawData["hcl"]
      val eyeColor = rawData["ecl"]
      val id = rawData["pid"]
      val countryId = rawData["cid"]

      if (birthYear == null || issueYear == null || expirationYear == null || height == null || hairColor == null || eyeColor == null || id == null)
        return null
    ...
    private fun parseYear(rawYear: String?): LocalDate? {
      return try {
        val format = DateTimeFormatterBuilder()
          ...
          .toFormatter()
        LocalDate.parse(rawYear, format)
      } catch (_: Exception) {
        null
      }
    }
