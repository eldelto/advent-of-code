...
func Test_parsePolicy(t *testing.T) {
	tests := []struct {
		...
		{"valid input", "1-3 a", &passwordPolicy{"a", 1, 3}, false},
		{"invalid input", "1-a3 !a", nil, true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := parsePolicy(tt.rawPolicy)
...
func Test_parsePolicy2(t *testing.T) {
	tests := []struct {
		name      string
		rawPolicy string
		want      pwdPolicy
		...
		{"valid input", "1-3 a", &passwordPolicy2{"a", 0, 2}, false},
		{"invalid input", "1-a3 !a", nil, true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := parsePolicy2(tt.rawPolicy)
			if (err != nil) != tt.wantErr {
				t.Errorf("parsePolicy() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
...
func Test_isValid2(t *testing.T) {
	tests := []struct {
		...
		{"invalid password0", "1-3 b", "cdefg", false},
		{"invalid password1", "2-9 c", "ccccccccc", false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			policy, err := parsePolicy2(tt.rawPolicy)
			assert.Equal(t, nil, err)

			got := policy.isValid(tt.pwd)
			assert.Equal(t, tt.want, got)
		})
	}
}

func TestFindValidPasswords(t *testing.T) {
	tests := []struct {
		name     string
		provider policyProvider
		wantErr  bool
	}{
		{"policy 1", parsePolicy, false},
		{"policy 2", parsePolicy2, false},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := findValidPasswords(parsedInput, tt.provider)
			if (err != nil) != tt.wantErr {
				t.Errorf("parsePolicy() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
