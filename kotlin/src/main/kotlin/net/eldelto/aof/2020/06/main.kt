package net.eldelto.aof.`2020`.`06`

fun main() {

  // Example
  val exampleGroups = exampleInput.split("\n\n").map(Group::parse)
  val exampleCount = exampleGroups.map { it.uniquePositiveAnswers().size }.reduce(Int::plus)
  println("Example result: $exampleCount")

  // Scenario 1
  val groups = input.split("\n\n").map(Group::parse)
  val count = groups.map { it.uniquePositiveAnswers().size }.reduce(Int::plus)
  println("Scenario 1 result: $count")

  // Scenario 2
  val unionCount = groups.map { it.commonPositiveAnswers().size }.reduce(Int::plus)
  println("Scenario 2 result: $unionCount")
}

data class Form(val positiveAnswers: Set<Char>)

data class Group(val forms: List<Form>) {

  companion object {
    fun parse(rawData: String): Group {
      val forms = rawData.split("\n")
        .map { Form(it.toSet()) }
  ...
  fun uniquePositiveAnswers(): Set<Char> = forms.flatMap(Form::positiveAnswers)
    .distinct()
    .toSet()

  fun commonPositiveAnswers(): Set<Char> = forms.map(Form::positiveAnswers)
